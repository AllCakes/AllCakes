...
# 서비스 회원 가입 및 탈퇴: 서비스 회원 가입은 각 서비스 회원 정보에 카카오계정 사용자 정보를 회원으로 저장하는 일입니다. 이는 연결과 별개의 처리입니다. 카카오계정으로 로그인한 사용자 정보를 서비스 서버에 회원 가입 처리하지 않으면 정상적인 가입 처리가 완료되지 않습니다.서비스 회원 탈퇴 또한 서비스가 자체적으로 구현해야 합니다. 연결 끊기는 카카오계정과 앱의 관계만 끊을 뿐, 서비스 서버에 저장된 회원 정보까지 지우지 않기 때문입니다
